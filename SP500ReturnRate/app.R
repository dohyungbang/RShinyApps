...
max_date <- Sys.Date() - 1
min_date <- ymd("2011-01-01")

tickers <- 
  read_csv("https://www.dropbox.com/scl/fi/sojq10xwwzbyou8vf8ary/SP500symbols.csv?rlkey=9cv2sjexpubntr16d2llcj0jn&dl=1")$Symbol
...
ui <- fluidPage(
  ...
  titlePanel("Historical Return Rates of S&P500 Companies (Source: Yahoo finance)"),
    ...
    mainPanel(
      ...
      fluidRow(column(6,
               ...
               column(6,
                      ...
                      fluidRow(box(h3("Indicator Definition", align = "Left"),
                                   h4("ExpReturn: Expected Return Rate (%)", style = "font-si13pt"),
                                   h4("SD: Standard Deviation (%)", style = "font-si13pt"),
                                   h4("CV: Coefficient of Variation", style = "font-si13pt"),
                                   h4("Min: Minimum Return Rate (%)", style = "font-si13pt"),
                                   h4("Max: Maximum Return Rate (%)", style = "font-si13pt"),
                                   h4("Corr: Correlation Coefficient (-1 to 1)", style = "font-si13pt"))
                      )
...
server <- function(input, output) {
  ...
  output$corr_plot <- renderPlot({
    ...
    ggplot(merged_df, aes(x = date, y = ReturnRate, color = Company)) + 
      ...
      ylab("Return Rate (%)") + 
      xlab("Date") + 
      ggtitle(paste0("Correlation of Return Rate")) + 
      corr_theme
  })
